...
var InternetOutDropRateByConnectionRegion = "InternetOutDropRateByConnectionRegion"

func init() {
	registerCollector(changeInternet(InternetOutDropRateByConnectionRegion, false), defaultEnabled, NewInternetOutDropRateByConnectionRegionCollector)
}

type internetOutDropRateByConnectionRegion struct {
	entries *prometheus.Desc
	log     log.Logger
}

func NewInternetOutDropRateByConnectionRegionCollector(logger log.Logger) (Collector, error) {
	label := snakeString(InternetOutDropRateByConnectionRegion)
	level.Info(logger).Log("label", label)
	return &internetOutDropRateByConnectionRegion{
		entries: prometheus.NewDesc(
			prometheus.BuildFQName(namespace, label, "value"),
			" region monitor with the drop amount of in and out ",
			[]string{"network"}, nil,
		),
		log: logger,
	}, nil
...
func (c *internetOutDropRateByConnectionRegion) Update(ch chan<- prometheus.Metric) error {
	defer func() {
		err := recover()
		if err != nil {
	...
	for _, v := range cfg.ConfigCollection.RegionMonitor.List {
		...
		in.Metrics = InternetOutDropRateByConnectionRegion
